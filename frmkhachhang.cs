...
namespace sotietkiem99
{
    public partial class frmkhachhang : Form
    {
        ...
        public void xoa()
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        int count = khachhangDAO.deleteByid(Convert.ToInt32(txtid.Text));
                        if (count > 0)
                        {
                            labelMessage.Text = count + "  được xóa!";
                            load(khachhangDAO.selectAll(), labelMessage.Text);
                        }
                        else
                        {
        ...
        private void search()
        {
            try
            {
                var list = khachhangDAO.search(textSearch.Text);
        ...
        public void luu()
        {
            try
            {
                khachhangModel doituong = new khachhangModel();
                ...
                doituong.ma = Convert.ToString(ma.Text.Trim());
                doituong.ngaysinh = Convert.ToString(ngaysinh.Text.Trim());
                doituong.thangsinh = Convert.ToString(thangsinh.Text.Trim());
                doituong.namsinh = Convert.ToString(namsinh.Text.Trim());
                
                doituong.ngaytao = DateTime.Now.ToString(); 
                //Convert.ToString(ngaytao.Text.Trim());
                doituong.dienthoai = Convert.ToString(dienthoai.Text.Trim());
                doituong.diachi = Convert.ToString(diachi.Text.Trim());
                doituong.email = Convert.ToString(email.Text.Trim());
                var gioitinhs = "";
                ...
                doituong.gioitinh = Convert.ToString(gioitinhs);

                doituong.cmnd = Convert.ToString(cmnd.Text.Trim());

                int count = khachhangDAO.insertOrUpdate(doituong);
                if (count > 0)
                {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    ...
                    ma.Text = row.Cells[3].Value.ToString();
                    ngaysinh.Text = row.Cells[4].Value.ToString();
                    thangsinh.Text = row.Cells[5].Value.ToString();
                    namsinh.Text = row.Cells[6].Value.ToString();
                    ngaytao.Text = row.Cells[7].Value.ToString();
                    dienthoai.Text = row.Cells[8].Value.ToString();
                    diachi.Text = row.Cells[9].Value.ToString();
                    email.Text = row.Cells[10].Value.ToString();
          
                    var gtinh= row.Cells[11].Value.ToString();
                    if (gtinh=="NAM")
                    {
                    ...
                    cmnd.Text = row.Cells[12].Value.ToString();
